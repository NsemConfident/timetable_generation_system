 ...
 * Constraints: no same-class exam at same time, room not double-booked,
 * optional supervisor (respect availability), multiple classes can have same subject at same time.
...
class ExamGenerator
{
    ...
    public function generate(int $assessmentSessionId): array
    {
        ...
        if ($session['type'] !== 'exam') {
            Response::error('This endpoint is for exam sessions only. Use CA generate for CA sessions.');
        }
        ...
        if (!$success) {
            Response::error(
                'Exam timetable generation failed: could not place all exams. ' .
                'Ensure enough slots and rooms, and that supervisor availability allows scheduling.'
            );
        }
    ...
    private function backtrack(int $demandIndex): bool
    {
        ...
        foreach ($attempts as $cell) {
            ...
            $this->place($assessmentSubjectId, $roomId, $dayId, $slotId, $supervisorId);
            if ($this->backtrack($demandIndex + 1)) {
                return true;
            }
            $this->unplace($assessmentSubjectId, $dayId, $slotId);
        }
        return false;
    }
    ...
    private function place(int $assessmentSubjectId, ?int $roomId, int $dayId, int $slotId, ?int $supervisorId): void
    {
    ...
    private function unplace(int $assessmentSubjectId, int $dayId, int $slotId): void
    {
